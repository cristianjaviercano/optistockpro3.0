    ...
    if ((gameMode === GameMode.Forklift || gameMode === GameMode.Tutorial) && fuel > 0) {
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        if (tile) {
          ...
          if (tile.buildingType === BuildingType.Pallet || (tile.pallets && tile.pallets[0])) {
            // Try to push pallet
            ...
            if (nextTile && driveableTypes.includes(nextTile.buildingType) && (!nextTile.pallets || !nextTile.pallets[0])) {
              ...
              setGrid(prev => {
                ...
                newGrid[nextTileY][nextTileX] = { ...nextTile, pallets: [true, false, false] };
                newGrid[tileY][tileX] = { ...tile, pallets: [false, false, false] };
                if (newGrid[tileY][tileX].buildingType === BuildingType.Pallet) {
                  newGrid[tileY][tileX].buildingType = BuildingType.Floor;
                }
                return newGrid;
              });
